...
import { RendererColors } from "../style/colors";
import { DrawOptions } from "../drawOptions";
import { BeeperRenderer, HorizontalFitterBeeperRenderer } from "../beeperRenderer";
...
class WorldRenderer {
    ...
    private _drawOptions: DrawOptions | null;
    private _beeperRenderer: BeeperRenderer;
    ...
    constructor(canvasContext: CanvasRenderingContext2D, style: RendererColors, scale: number) {
        ...
        this._mode = "normal";
        this._snapped = true;
        this._beeperRenderer = new HorizontalFitterBeeperRenderer(this.canvasContext);
    }
    ...
    private DrawBeepers() {
        ...
        for (let i =0; i < this.GetRowCount(); i++) {
            ...
            for (let j =0; j < this.GetColCount(); j++) {
                ...
                if (buzzers!==0) {
                    this._beeperRenderer.DrawBeeperSquare({
    ...
    private DrawGizmo() {
        if (!this._drawOptions || !this._drawOptions.cellGizmos) {
            return;
        }
        ...
        for (let i =0; i < this.GetRowCount(); i++) {
            ...
            for (let j =0; j < this.GetColCount(); j++) {
                ...
                let r = i + Math.floor(this._origin.r);
                let c = j + Math.floor(this._origin.c);
                if (!this._drawOptions.cellGizmos.has(r*MAX_LEN+c)) {
                    continue;
                }

                const gizmo = this._drawOptions.cellGizmos.get(r*MAX_LEN+c);
                if (gizmo?.color) {
                    ...
                    this.ColorCellBorder(
                        ...
                        {
                            north: this._drawOptions.cellGizmos.get((r+1)*MAX_LEN+c)?.color !== gizmo.color,
                            west: this._drawOptions.cellGizmos.get(r*MAX_LEN+(c-1))?.color !== gizmo.color,
                            south: this._drawOptions.cellGizmos.get((r-1)*MAX_LEN+c)?.color !== gizmo.color,
                            east: this._drawOptions.cellGizmos.get(r*MAX_LEN+(c+1))?.color !== gizmo.color
                        }
                    );                
                }
            }           
